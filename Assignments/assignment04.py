# Assignment 4 : Predict the output of the following python statements.

print("Value of e is %0.1f",2.7713)
print("Value of e is %0.1f" %2.713)
print("Programming in Python: Version %d" %3.5)
x=2
print("%20s : %d" % ("Python 3.0 is also known as Python",3000.57))
print("{0:2d} {1:3d} {2:4d}".format(x, x*x, x*x*x))

# ==============OUTPUT================== 
# Value of e is %0.1f 2.7713
# Value of e is 2.7
# Programming in Python: Version 
# Python 3.0 is also known as Python : 3000
#  2   4    8
